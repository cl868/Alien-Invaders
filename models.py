...
class Ship(GImage):
    ...
    def __init__(self):
        ...
        """
        super().__init__(x=GAME_WIDTH/2, y = SHIP_BOTTOM+(SHIP_HEIGHT/2), height = SHIP_HEIGHT,
        width = SHIP_WIDTH,source = "ship.png")
    # METHODS TO MOVE THE SHIP AND CHECK FOR COLLISIONS

    def shipcollides(self,bolt):
        """
        Determines if the alienbolt has collided with the ship.

        Parameter bolt: The laser bolt to check
        Precondition: bolt is of class Bolt
        """
        if bolt.isAlienBolts() == True:
...
class Alien(GImage):
    ...
    def __init__(self, x, y, source_image):
        """
        Initializes an alien

        Parameter x: x-coordinate of alien
        Precondition: x is a float

        Parameter y: y-coordinate of alien
        Precondition: y is a float

        Parameter source_image: image of alien
        Precondition: source_image is a string of the name of a valid png file
        """
...
class Bolt(GRectangle):
    ...
    def __init__(self,x,y,velocity):
        """
        Initializes the bolt object

        Parameter x: x-coordinate of bolt
        Precondition: x is a float

        Parameter y: y-coordinate of bolt
        Precondition: y is a float

        Parameter velocity: velocity of bolt
    ...
    def isPlayerBolt(self):
        """
        Checks to see if the bolt in question is a player bolt.
        """
        if self._velocity >= 0:
            return True

    def isAlienBolts(self):
        """
        Checks to see if the bolt in question is an alien bolt.
        """
        if self._velocity < 0:
            return True
